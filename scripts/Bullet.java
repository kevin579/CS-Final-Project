...
class Boom extends Bullet {
    ...
    public void explode() {
        for (Enemy enemy : GameFrame.enemys) {
            if (Math.sqrt(Math.pow(enemy.getX() - this.getX(), 2) + Math.pow(enemy.getY() - this.getY(), 2)) < this.getExplodeRadius()) {
...
class Missile extends Boom {
    ...
    public void move() {
        ...
        } else if (getParent().getTarget() != null && getParent().getTarget().getHp() > 0 && this.getExplodeTime() < 0) {
            this.dis = Math.sqrt(Math.pow((getParent().getTarget().getX() - this.getX()), 2) + Math.pow((getParent().getTarget().getY() - this.getY()), 2));
            this.setSpeedX(this.getSpeed() * (getParent().getTarget().getX() + getParent().getTarget().getWidth() / 2 - this.getX()) / (dis));
            this.setSpeedY(this.getSpeed() * (getParent().getTarget().getY() + getParent().getTarget().getHeight() / 2 - this.getY()) / (dis));
